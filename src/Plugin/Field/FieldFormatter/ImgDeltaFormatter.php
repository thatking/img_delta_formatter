...
/**
 * @file
 * Contains \Drupal\img_delta_formatter\Plugin\Field\FieldFormatter\ImgDeltaFormatter.
 */

namespace Drupal\img_delta_formatter\Plugin\Field\FieldFormatter;
 ...
 * )
 */
class ImgDeltaFormatter extends ImageFormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsSummary() {
    ...
    $deltas_setting = $this->getSetting('deltas');
    $deltas_reversed_setting = $this->getSetting('deltas_reversed');

    //$delta_label = strpos($deltas_setting['deltas'], ',') ? 'Deltas' : 'Delta';
    $delta_label = strpos($deltas_setting, ',') ? 'Deltas' : 'Delta';
    $deltas = t($delta_label . ': @deltas', array('@deltas' => trim($deltas_setting)));
    if (isset($deltas_reversed_setting) && $deltas_reversed_setting) {
      $deltas .= '. ' . t('Reversed.');
    }
    $summary[] = $deltas;
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
	...
	$deltas_setting = $this->getSetting('deltas');
    if (strpos($deltas_setting, ',')) {
      $deltas = explode(',', $deltas_setting);
      $deltas = array_map('trim', $deltas);
    }
    else {
      $delta = trim($deltas_setting);
      $deltas = array($delta);
    }
    // Reverse the items if needed.
	$deltas_reversed_setting = $this->getSetting('deltas_reversed');
    if ($deltas_reversed_setting) {
    ...
    foreach ($files as $delta => $file) {
      ...
      $elements[$delta] = array(
        '#theme' => 'img_delta_formatter',
        '#item' => $item,
        '#item_attributes' => $item_attributes,
        '#image_style' => $image_style_setting,
        '#deltas' => $deltas_setting,
        '#deltas_reversed' => $deltas_reversed_setting,
        '#url' => $url,
        '#cache' => array(
          'tags' => $cache_tags,
